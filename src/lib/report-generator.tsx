    ...
    <View>
      ...
      {items.map((it) => (
        <View key={it.item} style={s.checklistRow}>
          ...
          <Text style={[s.statusBadge, { backgroundColor: STATUS_COLOR[it.status] }]}>
...
function ConformityDocument({ analysis }: { analysis: AnalysisResult }) {
  const { identificacao_contrato: id, regularidade_fiscal_trabalhista: reg, instrucao_processual: inst, conclusao } = analysis;
  const decisaoColor = DECISAO_COLOR[conclusao.decisao_geral] ?? '#555';
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.header}>
          ...
          <Text style={s.subtitle}>
            EMSERH — Empresa Maranhense de Serviços Hospitalares | GCIF — Gerência de Controle Interno Financeiro
          </Text>
        </View>
        ...
        {/* Regularidade Fiscal */}
        <ChecklistSection
          title="2. REGULARIDADE FISCAL E TRABALHISTA (Itens 1–7)"
          items={reg}
        />

        {/* Instrução Processual */}
        <ChecklistSection
          title="3. INSTRUÇÃO PROCESSUAL (Itens 8–15)"
          items={inst}
        />

        {/* Conclusão */}
        <View style={[s.conclusaoBox, { borderColor: decisaoColor }]}>
          <Text style={[s.decisaoText, { color: decisaoColor }]}>
            DECISÃO: {conclusao.decisao_geral.replace('_', ' ')}
          </Text>
          <Text style={s.resumo}>{conclusao.resumo}</Text>

          <View style={s.totaisRow}>
            <View style={[s.totaisBox, { backgroundColor: '#16a34a' }]}>
              <Text style={s.totaisNum}>{conclusao.total_itens_conformes}</Text>
              <Text style={s.totaisLabel}>Conformes</Text>
            </View>
            <View style={[s.totaisBox, { backgroundColor: '#dc2626' }]}>
              <Text style={s.totaisNum}>{conclusao.total_itens_nao_conformes}</Text>
              <Text style={s.totaisLabel}>Não Conformes</Text>
            </View>
            <View style={[s.totaisBox, { backgroundColor: '#d97706' }]}>
              <Text style={s.totaisNum}>{conclusao.total_itens_atencao}</Text>
              <Text style={s.totaisLabel}>Atenção</Text>
            </View>
          </View>

          {conclusao.lista_pendencias.length > 0 && (
            <View style={s.pendencias}>
              {conclusao.lista_pendencias.map((p, i) => (
                <Text key={i} style={s.pendenciaItem}>• {p}</Text>
              ))}
            </View>
          )}
        </View>
        ...
        <View style={s.footer} fixed>
          <Text style={s.footerText}>
            Documento gerado automaticamente pelo Sistema de Auditoria EMSERH — GCIF | Base legal: Lei 13.303/2016, Portaria 439/2024-GAB/EMSERH, Portaria 279/2025-GAB/EMSERH
          </Text>
        </View>
      </Page>
    </Document>
...
export async function generateConformityReport(analysis: AnalysisResult): Promise<Buffer> {
  const uint8 = await renderToBuffer(<ConformityDocument analysis={analysis} />);
